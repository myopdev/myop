...
my $intergenic_short_length = 700; # $metapar{intergenic_length};
my $intergenic_long_length =  1e4;#$metapar{intergenic_length};
...
state_names = ("N", "Ns", "Nf", "I0","I1","I2", "EI0","EI1","EI2","ES", "Is0", "If0",   "Is1", "If1",  "Is2", "If2",
"ET0","ET1","ET2",  "rIs0", "rIf0",  "rIs1", "rIf1", "rIs2", "rIf2", "start", "stop", "acc0", "acc1", "acc2", "don0", "don1", "don2",
...
intron_model = "model/intergenic.model"
...
Ns = [observation = noncoding_model]
Nf = [observation = noncoding_model
      sequence_length = $intergenic_short_length]
I0 = [ observation = intron_model ]
Is0 = [ observation = intron_model
        duration = intron_length ]
If0 = [ observation = intron_model
        sequence_length = $intron_short_length ]
I1 = [ observation = intron_model ]
Is1 = [ observation = intron_model
        duration = intron_length ]
If1 = [ observation = intron_model
        sequence_length = $intron_short_length ]
I2 = [ observation = intron_model ]
Is2 = [ observation = intron_model
        duration = intron_length ]
If2 = [ observation = intron_model
        sequence_length = $intron_short_length ]
rI0 = [ observation = intron_model ]
rIs0 = [ observation = intron_model
        duration = intron_length ]
rIf0 = [ observation = intron_model
        sequence_length = $intron_short_length ]
rI1 = [ observation = intron_model ]
rIs1 = [ observation = intron_model
         duration = intron_length ]
rIf1 = [ observation = intron_model
        sequence_length = $intron_short_length ]
rI2 = [ observation = intron_model ]
rIs2 = [ observation = intron_model
        duration = intron_length ]
rIf2 = [ observation = intron_model
       sequence_length = $intron_short_length ]
...
EOF
open (GHMM, ">ghmm/model/ghmm_partial.model") or die "model/ghmm_partial.model: $!";
print GHMM $ghmm;
close(GHMM);
...
open (GHMM, "<ghmm/model/states.model") or die "model/states.model: $!";
my %estimatedTransitions;
while(<GHMM>) {
  chomp;
  my $line = $_;
  $line =~s/;//g;
  ...
  if($#parts >= 1) {
    ...
    if($#x >= 1) {
      $parts[0] = $x[1]."->".$x[0];
      $parts[1] =~ s/#.+//g;
      $estimatedTransitions{$parts[0]} = $parts[1];
    } else {
      $estimatedTransitions{$x[0]} = $parts[1];
    }
  }
}
close(GHMM);
...
$transitions{make_transition_str("don0", "If0")} = 1.0 - $pshort;
$transitions{make_transition_str("If0", "I0")} = 1.0;
$transitions{make_transition_str("I0", "I0")} = $p;
$transitions{make_transition_str("I0", "acc0")} = 1.0 - $p;
$transitions{make_transition_str("Is0", "acc0")} = 1.0;

$transitions{make_transition_str("don1", "Is1")} = $pshort;
$transitions{make_transition_str("don1", "If1")} = 1.0 - $pshort;
$transitions{make_transition_str("If1", "I1")} = 1.0;
$transitions{make_transition_str("I1", "I1")} = $p;
$transitions{make_transition_str("I1", "acc1")} = 1.0 - $p;
$transitions{make_transition_str("Is1", "acc1")} = 1.0;

$transitions{make_transition_str("don2", "Is2")} = $pshort;
$transitions{make_transition_str("don2", "If2")} = 1.0 - $pshort;
$transitions{make_transition_str("If2", "I2")} = 1.0;
$transitions{make_transition_str("I2", "I2")} = $p;
$transitions{make_transition_str("I2", "acc2")} = 1.0 - $p;
$transitions{make_transition_str("Is2", "acc2")} = 1.0;

$transitions{make_transition_str("racc0", "rIs0")} = $pshort;
$transitions{make_transition_str("racc0", "rIf0")} = 1.0 - $pshort;
$transitions{make_transition_str("rIf0", "rI0")} = 1.0;
$transitions{make_transition_str("rI0", "rI0")} = $p;
$transitions{make_transition_str("rI0", "rdon0")} = 1.0 - $p;
$transitions{make_transition_str("rIs0", "rdon0")} = 1.0;

$transitions{make_transition_str("racc1", "rIs1")} = $pshort;
$transitions{make_transition_str("racc1", "rIf1")} = 1.0 - $pshort;
$transitions{make_transition_str("rIf1", "rI1")} = 1.0;
$transitions{make_transition_str("rI1", "rI1")} = $p;
$transitions{make_transition_str("rI1", "rdon1")} = 1.0 - $p;
$transitions{make_transition_str("rIs1", "rdon1")} = 1.0;

$transitions{make_transition_str("racc2", "rIs2")} = $pshort;
$transitions{make_transition_str("racc2", "rIf2")} = 1.0 - $pshort;
$transitions{make_transition_str("rIf2", "rI2")} = 1.0;
...
$transitions{make_transition_str( "Ns", "start" )} = $pforward * ((1.0/$intergenic_short_length)  - $final_state_prob);
$transitions{make_transition_str( "Ns", "rstop" )} = (1.0 - $pforward) * ((1.0/$intergenic_short_length) - $final_state_prob);
$transitions{make_transition_str( "N", "start" )} = $pforward * ((1.0/$intergenic_long_length)  - $final_state_prob);
$transitions{make_transition_str( "N", "rstop" )} = (1.0 - $pforward) * ((1.0/$intergenic_long_length) - $final_state_prob);
$transitions{make_transition_str("N", "N")} = 1.0 - (1.0/$intergenic_long_length);
$transitions{make_transition_str("Nf", "N")} = 1.0 ;
$transitions{make_transition_str("Ns", "Ns")} = 1.0 - (1.0/$intergenic_short_length);
$transitions{make_transition_str( "stop", "Nf" )} = 0.8;
$transitions{make_transition_str( "stop", "Ns" )} = 0.2;
$transitions{make_transition_str( "rstart", "Nf" ) }= 0.8;
$transitions{make_transition_str( "rstart", "Ns" ) }= 0.2;

$transitions{make_transition_str("ES","stop")} = 1.0;
$transitions{make_transition_str("start","ES")} = $estimatedTransitions{"start->ES"};
$transitions{make_transition_str("rstop","rES")} = $estimatedTransitions{"rstop->rES"};
$transitions{make_transition_str("start", "EI0")} = $estimatedTransitions{"start->EI0"};
$transitions{make_transition_str("start", "EI1")} =$estimatedTransitions{"start->EI1"};
$transitions{make_transition_str("start", "EI2")} =$estimatedTransitions{"start->EI2"};
$transitions{make_transition_str("rstop","rET0")} = $estimatedTransitions{"rstop->rET0"};
$transitions{make_transition_str("rstop","rET1")} =$estimatedTransitions{"rstop->rET1"};
$transitions{make_transition_str("rstop","rET2")} = $estimatedTransitions{"rstop->rET2"};
...
$transitions{make_transition_str("acc0", "ET0")} = $estimatedTransitions{"acc0->ET0"};
$transitions{make_transition_str("acc0", "E00")} =  $estimatedTransitions{"acc0->E00"};
$transitions{make_transition_str("acc0", "E01")} =  $estimatedTransitions{"acc0->E01"};
$transitions{make_transition_str("acc0", "E02")} =  $estimatedTransitions{"acc0->E02"};


$transitions{make_transition_str("acc1", "ET1")} = $estimatedTransitions{"acc1->ET1"};
$transitions{make_transition_str("acc1", "E10")} =  $estimatedTransitions{"acc1->E10"};
$transitions{make_transition_str("acc1", "E11")} =  $estimatedTransitions{"acc1->E11"};
$transitions{make_transition_str("acc1", "E12")} =  $estimatedTransitions{"acc1->E12"};


$transitions{make_transition_str("acc2", "ET2")} = $estimatedTransitions{"acc2->ET2"};
$transitions{make_transition_str("acc2", "E20")} =  $estimatedTransitions{"acc2->E20"};
$transitions{make_transition_str("acc2", "E21")} =  $estimatedTransitions{"acc2->E21"};
$transitions{make_transition_str("acc2", "E22")} =  $estimatedTransitions{"acc2->E22"};
...
$transitions{make_transition_str("rdon1","rEI0")} =$estimatedTransitions{"rdon1->rEI0"};
$transitions{make_transition_str("rdon1","rE00")} =$estimatedTransitions{"rdon1->rE00"};
$transitions{make_transition_str("rdon1","rE01")} =$estimatedTransitions{"rdon1->rE01"};
$transitions{make_transition_str("rdon1","rE02")} =$estimatedTransitions{"rdon1->rE02"};

$transitions{make_transition_str("rdon0","rEI2")} =$estimatedTransitions{"rdon0->rEI2"};
$transitions{make_transition_str("rdon0","rE20")} =$estimatedTransitions{"rdon0->rE20"};
$transitions{make_transition_str("rdon0","rE21")} =$estimatedTransitions{"rdon0->rE21"};
$transitions{make_transition_str("rdon0","rE22")} =$estimatedTransitions{"rdon0->rE22"};

$transitions{make_transition_str("rdon2","rEI1")} =$estimatedTransitions{"rdon2->rEI1"};
$transitions{make_transition_str("rdon2","rE10")} =$estimatedTransitions{"rdon2->rE10"};
$transitions{make_transition_str("rdon2","rE11")} =$estimatedTransitions{"rdon2->rE11"};
$transitions{make_transition_str("rdon2","rE12")} =$estimatedTransitions{"rdon2->rE12"};
...
open (GHMM, "<ghmm/model/ghmm_partial.model") or die "$!";
my $reading_trans = 0;
my @states =  ("N","I0","I1","I2",
...
foreach my $line (<GHMM>) {
  my $prob = 1.0/(scalar(@states));
  if ($line =~ /initial_probabilities =/) {
    print GHMM2 "initial_probabilities = ( ";
    my $sum = 0.0;
    foreach my $state (@states) {
      if(defined $estimatedTransitions{$state} && $estimatedTransitions{$state} > 0) {
	$sum += $prob;
      }
    }
    ...
    foreach my $state (@states) {
      if(defined $estimatedTransitions{$state} && $estimatedTransitions{$state} > 0) {
	print GHMM2 "\"$state\": ".($prob/$prob).";\n";
      }
    }
 print GHMM2 ")\n";
  } elsif ($line =~ m/transitions =/) {
    ...
    print GHMM2 ")\n";
  } elsif ($line =~ m/terminal_probabilities = /) {
    my $prob = 1.0/$#states;
    print GHMM2 "terminal_probabilities = ( ";
    foreach my $state (@states) {
	print GHMM2 "\"$state\": ".$prob.";\n";
    }
    print GHMM2 ")\n";
...
system("rm ghmm/model/ghmm_partial.model");
system("mv ghmm/model/ghmm_final.model ghmm/model/ghmm_partial.model");
